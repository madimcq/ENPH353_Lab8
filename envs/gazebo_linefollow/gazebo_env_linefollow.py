...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def get_tile(self,image,left,right):
        tile = [[]]*image.shape[0]
    ...
    def process_image(self, data):
        ...
        # cv_image shape: 240(x), 320(y), 3(colour channels)

        # PLAN: whichever tile has the most 100 < cv_image[i][j] < 130 values is the state 1 tile

        height, width = cv_image.shape
        # create tile spacing for tile generation
        tile_space = []
        for n in range(0,11):
           spacing = int(n*width/10)
           tile_space.append(spacing)

        # determine how much 'blue' (i.e. how much line) each tile contains
        amnt_blue = []
        for n in range(0,10):
            blue = 0
            for y_val in cv_image[:,tile_space[n]:tile_space[n+1]]:
               for x_val in y_val:
                    if x_val < 130 and x_val > 100:
                       blue += 1
            amnt_blue.append(blue)

        # create state array
        max_blue = max(amnt_blue)
        state[amnt_blue.index(max_blue)] = 1
        
        print("cv_image: ", state)
        print("amount blue: ", amnt_blue)
